...
void xpl::type_checker::do_neg_node(cdk::neg_node * const node, int lvl) {
  //TODO: Only verifies integers for now
...
void xpl::type_checker::do_sweep_plus_node(xpl::sweep_plus_node * const node, int lvl){
 if (node->left() == nullptr)
    throw std::string("left doesnt exist");
 if (node->inicialization() == nullptr)
    throw std::string("inicialization doesnt exist");
 if (node->condition() == nullptr)
    throw std::string("condition doesnt exist");
 if (node->increment() == nullptr)
    throw std::string("increment doesnt exist");
 if (node->block() == nullptr)
    throw std::string("block doesnt exist");
...
void xpl::type_checker::do_variable_declaration_node(xpl::variable_declaration_node * const node, int lvl){
 std::string &name = *node->name();
  std::shared_ptr<xpl::symbol> symbol =  std::make_shared<xpl::symbol>(node->type(), name, 0);
    ...
    exit(0);
 	...
 	if (!_symtab.find(name))	{
    	_symtab.insert(name,symbol);
    }
}
//---------------------------------------------------------------------------
void xpl::type_checker::do_variable_definition_node(xpl::variable_definition_node * const node, int lvl){	
 std::string &name = *node->name();
  std::shared_ptr<xpl::symbol> symbol =  std::make_shared<xpl::symbol>(node->type(), name, 0);
...
//---------------------------------------------------------------------------
void xpl::type_checker::do_memory_position_node(xpl::memory_position_node * const node, int lvl){
//TODO
}
