...
namespace xpl {
  ...
  protected:
    ...
    void process_name(std::string tag, int lvl){
          os() << std::string(lvl, ' ') << "<id>" << tag << "</id>" << std::endl;
    }
    void process_type(basic_type* t, int lvl){
        os() << std::string(lvl, ' ') << "<type>" << t->name()<<std::endl;
        if(t->subtype()){
            process_type(t->subtype(),lvl+2);
        }
        os()<<std::string(lvl, ' ')<<"</type>"<< std::endl;
    }
  public:
  ...
  public:
    ...
    void do_variable_definition_node(xpl::variable_definition_node * const node, int lvl);
    void do_null_node(xpl::null_node * const node, int lvl);
    void do_memory_position_node(xpl::memory_position_node * const node, int lvl); 
